...
void fprintperm(FILE *fp, int p[]) {
  int i;
  for(i = 1; i < n*n; i++) fprintf(fp, "%d ", p[i]-1);
  fprintf(fp, "%d\n", p[n*n]-1);
}
...
void dfs(FILE *fp, int code[], int depth, int trans_perms[][n*n+1], int inv_perms[][n*n+1]) {
  int i,j;
  int p[n*n+1];
  perm(code, p);
  if(ok(p)) {
    if(leftmost(p, trans_perms, inv_perms)) {
      if(depth == n*n-1) fprintperm(fp, p);
      else {
        for(i = 1; i < n*n+1-depth; i++) {
          code[depth] = i;
          for(j = depth+1; j < n*n-1; j++) code[j] = 0;
          dfs(fp, code, depth+1, trans_perms, inv_perms);
        }
      }
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  char outfile[64];
  if(argc != 4) {
    printf("Usage repspart a b c\n");
    return EXIT_SUCCESS;
  }
  time(&start);
  for(i = 0; i < n*n-1; i++) code[i] = 0;
  sscanf(argv[1], "%d", &code[0]);
  sscanf(argv[2], "%d", &code[1]);
  sscanf(argv[3], "%d", &code[2]);
  sprintf(outfile, "%02d%02d%02dyb.txt", code[0], code[1], code[2]);
  fp = fopen(outfile, "w");
  if(fp == NULL) {
  ...
  code[n*n-1] = 1;
  //Generate the permutations corresponding to a tensor product
  //G \otimes G. We will find all solutions modulo these.
  ...
  dfs(fp, code, 3, trans_perms, inv_perms);
  time(&finish);
  fprintf(fp, "Duration: %.0lf seconds.\n", difftime(finish, start));
  fclose(fp);
  return EXIT_SUCCESS;
}
